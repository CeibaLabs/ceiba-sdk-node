...
## 2026-07-23 — Continuous CI workflow + honest README badge/nav header (`feat/sdk-release-readiness-and-readme`)

- **Scope:** two genuinely Builder-sized gaps scoped alongside the still-operator-led first npm publish (`_workspace/next.md`'s "New work stream 3"): a real continuous-CI workflow (the only existing workflow, `release.yml`, is tag-triggered only and never runs on ordinary pushes, so there was no "tests pass" signal outside of a release), and a badge/nav-link header on `README.md`. This branch does **not** perform the actual first npm publish — that stays David's own, separate action (npm Trusted Publisher registration on npmjs.com, a version-number decision, one `git tag` push).
- **`.github/workflows/ci.yml` (new file):** triggers on `push`/`pull_request` to `dev` and `main`; runs `npm ci` → `npm run typecheck` → `npm test` → `npm run build` → `npm run smoke:package`, mirroring `release.yml`'s own step order and script reuse. Deliberately omits `registry-url` on `actions/setup-node` and `permissions: id-token: write` — this workflow never publishes, so it needs neither. Confirmed structurally against `release.yml` directly (not just eyeballed): same `runs-on: ubuntu-latest`, same `actions/checkout@v4`/`actions/setup-node@v4` versions.
- **`README.md` header:** exactly four badges (CI — GitHub's own native workflow-status badge, no shields.io/no logo; npm version — shields.io, `%40ceibalabs%2Fceiba-sdk`; License — static MIT badge; Node engine — static `>=20` badge) plus one nav-link line (Docs · Quickstart · Examples · Issues), added directly under the `# Ceiba SDK` heading. `git diff` confirms the change is purely additive at the top — zero deletions or modifications anywhere else in the file.
- **Honesty constraint, explicitly checked, not just assumed:** the npm version badge will legitimately show "not found" until the real publish happens — expected, not a bug, not something this slice fixes. No coverage badge (no coverage tool configured anywhere in this repo), no Discord badge (doesn't exist), no `llms.txt` link (doesn't exist), no stars/trending badge (no real traction yet, pre-launch), no `logo=` parameter on any shields.io badge, no ASCII-art banner. The existing `docs.useceiba.com` links are kept exactly as they already were (not a new addition) — no badge or text asserts the docs site is reachable, since it's confirmed via `dig` to still not resolve in DNS (a known, separately-tracked operator blocker in `blockers.md`).
- **Verified locally, in the exact order `ci.yml` specifies, before it ever runs on GitHub's infrastructure:** `npm ci`, `npm run typecheck`, `npm test` (3 passing), `npm run build`, `npm run smoke:package` — all pass. YAML sanity-checked via `python3`/`PyYAML` (parses cleanly; `on:`/`jobs:` shape confirmed, including the well-known PyYAML "Norway problem" where a bare `on:` key parses as boolean `True` rather than the string `"on"` — a parser quirk, not a workflow bug; GitHub's own Actions parser handles it correctly, and `release.yml` has always had the identical bare `on:` key). Badge URL correctness verified by decoding the npm badge's URL-encoded package name (`%40ceibalabs%2Fceiba-sdk` → `@ceibalabs/ceiba-sdk`, exact match) and the Node-engine badge's encoded value (`%3E%3D20` → `>=20`, exact match against `package.json`'s `engines.node`).
- `npm run typecheck`, `npm run build`, `git diff --check e58743c..HEAD` all pass. Diff is exactly the two intended files (`ci.yml` new, `README.md` header-only).
- No SDK source, adapter, export, `package.json`, or `release.yml` change in this branch. No real `npm publish`, no version bump, no tag created — those remain David's own action.
